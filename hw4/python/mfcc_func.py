...
def _mfcc_13_per_frame(x, sr, n_fft, n_mels, n_mfcc):
    import librosa
    x = x / 32768.0
...
def create_mfcc_features(record_list, FFTSize, sample_rate, numOfMelFilters, numOfDctOutputs):
    feats=[]
    labels=[]
    for root, fname in record_list:
        path=os.path.join(root, fname)
        sr, x = _read_wav_8k(path, sample_rate)
        m = _mfcc_13_per_frame(x, sr, FFTSize, numOfMelFilters, numOfDctOutputs)
        mu = np.mean(m, axis=1)
        sd = np.std(m, axis=1)
